...
import { Bars3Icon, Bars4Icon, ChevronRightIcon, XMarkIcon } from '@heroicons/react/24/outline';
      ...
      <nav aria-label="Global" className="mt-12">
        <div className="flex lg:hidden ml-4">
          <button type="button" onClick={() => setMobileMenuOpen(true)}>
            <Bars3Icon aria-hidden="true" className="h-6 w-6 border-none outline-none" />
          </button>
        </div>
      </nav>
      <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
        ...
        <DialogPanel className="fixed inset-y-0 left-0 z-10 w-[60%] overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div className="flex items-center justify-between mr-4 ">
            ...
            >
              <XMarkIcon aria-hidden="true" className="h-4 w-4 border-none outline-none" />
            </button>
          </div>
